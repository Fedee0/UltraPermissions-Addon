...
public class RemovePermissionFromGroup extends Element {
    public RemovePermissionFromGroup(UltraCustomizer plugin) {
        super(plugin);
    }
    ...
    public String[] getDescription() {
        return new String[]{"Allows you to remove a permission from a group", "(UltraPermissions)"};
    }
    ...
    public void run(ElementInfo info, ScriptInstance instance) {
        try {
            ...
            if (api.getGroups().name(groupName).isPresent()) {
                ...
                group.getPermissions().name(permission).get(0).remove();
            } else {
                Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', "&4[ERROR] &c- A Group/Permission with this name don't exist!"));
            }
